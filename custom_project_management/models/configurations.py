...
class ChecklistAllocationLine(models.Model):
    ...
    checklist_allocation_floor_id = fields.Many2one(
        'checklist.allocation', 'CK Floor')
    checklist_allocation_flat_id = fields.Many2one(
        'checklist.allocation', 'CK Flat')
    ...
    project_activity_name_flat_id = fields.Many2one(
        'project.activity.name', string='Flat Activity Name')  # for flat
    project_activity_name_floor_id = fields.Many2one(
        'project.activity.name', string='Floor Activity Name')  # for Floor
    ...
    is_created = fields.Selection(
        [('yes', 'Yes'), ('no', 'No')], default="no", string='Is Created')
        ...
        if self.floor_ids:
            ...
            for allocation_line in self.chk_floor_id.checklist_allocation_floor_ids:
                ...
                for floor_line in self.floor_ids:
                    floor = floor_line
                    act_created = project_activity_obj.search([('project_id', '=', floor.project_id.id), (
                        'floor_id', '=', floor.project_floor_id.id), ('project_activity_name_id', '=', activity.id), ('tower_id', '=', tower.id)])
                    if not act_created:
                        project_activity_data = {'project_activity_name_id': activity.id, 'description': activity.description, 'name': activity.name,
                                                 'floor_id': floor.project_floor_id.id, 'tower_id': floor.tower_id.id, 'project_id': floor.project_id.id, 'floor_id': floor.id}
                        activity_rec = project_activity_obj.create(
                            project_activity_data)
                        allocation_line.is_created = 'yes'
                        for activity_type in activity.panl_ids:
                            project_activity_type_data = {'activity_id': activity_rec.id, 'project_actn_id': activity_type.patn_id.id,
                                                          'name': activity_type.patn_id.name, 'project_id': floor.project_id.id, 'tower_id': floor.tower_id.id, 'floor_id': floor.id}
                            activity_type_re = project_activity_type_obj.create(
                                project_activity_type_data)
                            checklist_data = []
                            for chk in activity_type.patn_id.patnl_ids:
                                checklist_data.append(
                                    {'activity_type_id': activity_type_re.id, 'checklist_template_id': chk.checklist_id.id})
                            project_checklist_line_obj.create(checklist_data)
                        rec = checklist_allocation_line_obj.search(
                            [('floor_id', '=', floor.id), ('chk_flat_id', '=', self.id)])
                        rec.is_created = 'yes'
                        self.is_created = 'yes'
        return
        ...
        if self.flat_ids:
            ...
            for allocation_line in self.chk_flat_id.checklist_allocation_flat_ids:
                ...
                for flat_line in self.flat_ids:
                    flat = flat_line
                    act_created = project_activity_obj.search([('project_id', '=', flat.project_id.id), (
                        'flat_id', '=', flat.project_flat_id.id), ('project_activity_name_id', '=', activity.id), ('tower_id', '=', tower.id)])
                    if not act_created:
                        project_activity_data = {'project_activity_name_id': activity.id, 'description': activity.description, 'name': activity.name,
                                                 'flat_id': flat.project_flat_id.id, 'tower_id': flat.tower_id.id, 'project_id': flat.project_id.id, 'floor_id': flat.floor_id.id or ''}
                        activity_rec = project_activity_obj.create(
                            project_activity_data)
                        allocation_line.is_created = 'yes'
                        for activity_type in activity.panl_ids:
                            project_activity_type_data = {'activity_id': activity_rec.id, 'project_actn_id': activity_type.patn_id.id, 'name': activity_type.patn_id.name,
                                                          'project_id': flat.project_id.id, 'tower_id': flat.tower_id.id, 'flat_id': flat.project_flat_id.id, 'floor_id': flat.floor_id.id or ''}
                            activity_type_re = project_activity_type_obj.create(
                                project_activity_type_data)
                            checklist_data = []
                            for chk in activity_type.patn_id.patnl_ids:
                            ...
                            project_checklist_line_obj.create(checklist_data)
                        rec = checklist_allocation_line_obj.search(
                            [('flat_id', '=', flat.id), ('chk_flat_id', '=', self.id)])
                        rec.is_created = 'yes'
                        self.is_created = 'yes'
        return
...
class ChecklistAllocation(models.Model):
    ...
    checklist_allocation_floor_ids = fields.One2many(
        'checklist.allocation.line', 'checklist_allocation_floor_id')
    checklist_allocation_flat_ids = fields.One2many(
        'checklist.allocation.line', 'checklist_allocation_flat_id')
    ...
    def action_load_floor_activity(self):
        ...
        for id in activity_names:
            checklist_al.append(
                {'checklist_allocation_floor_id': self.id, 'project_activity_name_floor_id': id})
        if checklist_al:
    ...
    def action_sync_floors(self):
        ...
        if self.tower_id:
            ...
            tower = self.tower_id
            floors = self.env['project.floors'].search(
                [('tower_id', '=', tower.id)]).ids
            if floors:
                group_size = 5
                groups = []
                ...
                for data in groups:
                    checklist_allocation_line_obj.create({
                        ...
                        # Replace record_id_1 and record_id_2 with actual record ids
                        'floor_ids': [(6, 0, data)]
                    })
        return
    ...
    def action_sync_flats(self):
        ...
        if self.tower_id:
            ...
            tower = self.tower_id
            flats = self.env['project.flats'].search(
                [('tower_id', '=', tower.id)]).ids
            if flats:
                group_size = 5
                groups = []
    ...
    def action_load_flat_activity(self):
        ...
        for id in activity_names:
            checklist_al.append(
                {'checklist_allocation_flat_id': self.id, 'project_activity_name_flat_id': id})
...
class ProjectTower(models.Model):
    ...
    project_details_line_id = fields.Many2one(
        'project.details', 'Project Details Line Id')
    flat_id = fields.Many2one('project.flats', 'Flats')
    floor_id = fields.Many2one('project.floors', 'Floors')
    tower_floor_line_id = fields.One2many(
        'project.floors', 'tower_id', string="Tower Floor")
    tower_flat_line_id = fields.One2many(
        'project.flats', 'tower_id', string="Tower Flat")

    image = fields.Binary('Image')
    assigned_to_ids = fields.Many2many('res.users')
    project_details_line = fields.One2many(
        'project.details', 'project_info_id')
    project_info_tower_line = fields.One2many(
        'project.info.tower', 'project_info_tower_id')
    ...
    @api.model
    def total_count_from_status_scheduler(self):
        records = self.search([])  # or use a domain to limit projects
        records._compute_total_count_from_status_tower()
    ...
    @api.depends(
        'tower_flat_line_id.flats_nc', 'tower_flat_line_id.flats_orange',
        'tower_flat_line_id.flats_yellow', 'tower_flat_line_id.flats_red', 'tower_flat_line_id.flats_green',
        'tower_floor_line_id.floors_nc', 'tower_floor_line_id.floors_orange',
        'tower_floor_line_id.floors_yellow', 'tower_floor_line_id.floors_red', 'tower_floor_line_id.floors_green'
    )
    def _compute_total_count_from_status_tower(self):
        _logger.info("=====compute total count from status tower=====")
        for record in self:
            flat_lines = record.tower_flat_line_id or self.env['project.flat.line']
            floor_lines = record.tower_floor_line_id or self.env['project.floor.line']

            # Always assign values, even if 0
            record.project_nc = (
                sum(flat_lines.mapped('flats_nc') or [0]) +
                sum(floor_lines.mapped('floors_nc') or [0])
            )
            record.project_orange = (
                sum(flat_lines.mapped('flats_orange') or [0]) +
                sum(floor_lines.mapped('floors_orange') or [0])
            )
            record.project_yellow = (
                sum(flat_lines.mapped('flats_yellow') or [0]) +
                sum(floor_lines.mapped('floors_yellow') or [0])
            )
            record.project_red = (
                sum(flat_lines.mapped('flats_red') or [0]) +
                sum(floor_lines.mapped('floors_red') or [0])
            )
            record.project_green = (
                sum(flat_lines.mapped('flats_green') or [0]) +
                sum(floor_lines.mapped('floors_green') or [0])
            )
    ...
    def _compute_tower_progress_bar(self):
        for rec in self:
            ...
            if rec.tower_floor_line_id or rec.tower_flat_line_id:
                total_line = len(rec.tower_floor_line_id.filtered(
                    lambda obj: obj.floor_progress_percentage != 0.0)) + \
                    len(rec.tower_flat_line_id.filtered(
                        lambda obj: obj.flats_progress_percentage != 0.0))
                count_percent = sum(rec.tower_floor_line_id.mapped(
                    'floor_progress_percentage')) + sum(rec.tower_flat_line_id.mapped('flats_progress_percentage'))
                total_percent = total_line * 100
                if total_percent > 0:
                    rec.tower_progress_percentage = round(
                        (count_percent / total_percent) *
                        100, 2  # Correct rounding syntax
                    )
    ...
    def get_project_towers(self, project_info_id):
        ...
        tower_data = self.search_read(
            [('project_id', '=', project_info_id)], ['id', 'name'])
        return tower_data
...
class ProjectFlats(models.Model):
    ...
    project_activity_name_id = fields.Many2one(
        'project.activity.name', 'Activity Name')
    ...
    # @api.model
    # def run_flag_in_flat(self):
    #     records = self.search([])  # or use a domain to limit projects
    #     records._compute_total_count_from_status_flats()
    ...
    def _compute_total_count_from_status_flats(self):
        for record in self:
            if record.activity_ids:
                nc = sum(record.activity_ids.mapped('act_nc'))
                orange = sum(record.activity_ids.mapped('act_orange'))
                yellow = sum(record.activity_ids.mapped('act_yellow'))
                red = sum(record.activity_ids.mapped('act_red'))
                green = sum(record.activity_ids.mapped('act_green'))

                record.flats_nc = nc
                record.flats_orange = orange
                record.flats_yellow = yellow
                record.flats_red = red
                record.flats_green = green

                # _logger.info("Values NC: %s", nc)
                # _logger.info("Values Orange: %s", orange)
                # _logger.info("Values Yellow: %s", yellow)
                # _logger.info("Values Red: %s", red)
                # _logger.info("Values Green: %s", green)

    def _compute_flats_progress_bar(self):
        for rec in self:
            rec.flats_progress_percentage = 0
            if rec.activity_ids:
                total_line = len(rec.activity_ids.filtered(
                    lambda obj: obj.progress_percentage != 0.0))
                count_percent = sum(
                    rec.activity_ids.mapped('progress_percentage'))
                total_percent = total_line * 100
                if total_percent > 0:
                    rec.flats_progress_percentage = round(
                        (count_percent / total_percent) *
                        100  # Removed tuple creation
                    )
...
class ProjectFloors(models.Model):
    ...
    p_activity_id = fields.Many2one(
        'project.activity', 'Activity Id', readonly="1")
    ...
    # @api.model
    # def run_flag_in_floor(self):
    #     records = self.search([])  # or use a domain to limit projects
    #     records._compute_total_count_from_status_floors()
    ...
    def _compute_total_count_from_status_floors(self):
        for record in self:
            if record.activity_ids:
                ...
                record.floors_nc = nc
                record.floors_orange = orange
                record.floors_yellow = yellow
                record.floors_red = red
                record.floors_green = green

                # _logger.info("Computed floor values for record %s - NC: %s, Orange: %s, Yellow: %s, Red: %s, Green: %s",
                #             record.id, nc, orange, yellow, red, green)
    ...
    def _compute_floor_progress_bar(self):
        for rec in self:
            ...
            if rec.activity_ids:
                ...
                total_percent = total_line * 100
                if total_percent > 0:
                    rec.floor_progress_percentage = round(
                        (count_percent / total_percent) * 100)
                    rec.tower_id._compute_tower_progress_bar()
...
class ProjectActivity(models.Model):
    ...
    project_id = fields.Many2one(
        'project.info', string='Project', compute="_compute_project_id", store=True)
    tower_id = fields.Many2one(
        'project.tower', string='Tower', compute="_compute_tower_id", store=True)
    ...
    status = fields.Selection([('draft', 'Draft'), ('submit', 'Submit'), (
        'checked', 'Checked'), ('approve', 'Approved'), ('approver_reject', 'Approver Rejected'), ('checker_reject', 'Checker Rejected')], default='draft', string="Status")
    ...
    def _compute_total_count_from_status_act(self):
        _logger.info("======compute count from status act====from project activity")
        for record in self:
    ...
    def _compute_progress_bar(self):
        for rec in self:
            ...
            total_lines = len(activity_type_ids.filtered(
                lambda obj: obj.progress_percentage != 0.0))
            count = sum(1 for line in activity_type_ids if line.status not in [
                        'draft', 'submit'])
            rec.progress_percentage = (
                count / total_lines) * 100 if total_lines > 0 else 0
            rec.floor_id._compute_floor_progress_bar()
            rec.flat_id._compute_flats_progress_bar()

class ProjectActivityTypeImage(models.Model):
    ...
    img_desc = fields.Char("Image Description")
    url = fields.Char("URL")
    img_type = fields.Selection(
        [('pat', 'PAT'), ('other', 'Other')], default='pat', string="Image Type")
...
class ProjectActivityType(models.Model):
        ...
        for rec in self:
            if rec.activity_id and rec.activity_id.floor_id and rec.activity_id.floor_id.tower_id and rec.activity_id.floor_id.tower_id.project_id:
                rec.display_name = str(
                    rec.activity_id.floor_id.tower_id.project_id.name) + "-"+str(rec.name)
                ...
                rec.display_name = str(
                    rec.activity_id.flat_id.tower_id.project_id.name) + "-" + str(rec.name)
            else:
                rec.display_name == rec.name

    name = fields.Char('Name')
    seq_no = fields.Char('Sequence', readonly=1, store=True)
    activity_type_img_ids = fields.One2many(
        'project.activity.type.image', 'activity_type_id')
    project_activity_type_id = fields.Many2one(
        'project.activity.type', 'Activity Type')
    project_actn_id = fields.Many2one(
        'project.activity.type.name', 'Activity Type Name')
    overall_remarks = fields.Char()
    overall_remarks_maker = fields.Char('Overall Maker')
    overall_remarks_checker = fields.Char('Overall Checker')
    overall_remarks_approver = fields.Char('Overall Approver')
    img_desc = fields.Char("Image Description")
    ...
    # attachments_ids = fields.Many2many('ir.attachments', 'Attachments')
    # project_activity_id = fields.Many2one('project.activity','Activity Id')
    image_ids = fields.Many2many(
        'ir.attachment', 'project_info_image_rel', 'project_info_id', 'image_id', string='Images')
    activity_image = fields.Binary("Image")
    checklist_ids = fields.One2many(
        'project.checklist.line', 'activity_type_id')
    # pcl_log_ids = fields.One2many('project.checklist.line.log','activity_type_id')
    ...
    status = fields.Selection([('draft', 'Draft'),
                              ('submit', 'Submit'),
                               ('checked', 'Checked'),
                               ('approve', 'Approved'), ('checker_reject',
                                                         'Checker Rejected'),
    ...
    act_responsible_person = fields.Many2one(
        'res.partner', 'Responsible Person')
    ...
    @api.model
    def run_totalcount_status(self):
        records = self.search([])  # or use a domain to limit projects  
        records._compute_total_count_from_status_act_type()
    ...
    def get_c_m_a_data(self, project_id, tower):
        ...
        if tower:
            ...
            project_list = self.env['project.info'].project_name_list(
                project_id, tower)
        ...
        for activity in projects:
            if activity.status == 'draft':
                ...
                counts['Approver'] += 1
            elif activity.status == 'checker_reject':
                counts['Maker'] += 1
            elif activity.status == 'approver_reject':
                counts['Checker'] += 1
    ...
    def action_submit(self):
        ...
        records.write(
            {'status': 'submit', 'submitted_date': fields.Datetime.now()})
    ...
    def action_check(self):
        ...
        if checker_id:
            # Perform your operation on the selected records
            records.write({'status': 'checked', 'type_status': 'checked',
                          'user_checker': checker_id, 'checked_date': fields.Datetime.now()})
    ...
    def action_approve(self):
        ...
        if approver_id:
            # Perform your operation on the selected records
            _logger.info(
                "--action_approve---active_ids--------%s", len(records))
            
            records.write({'status': 'approve', 'type_status': 'approve',
                          'user_approver': approver_id, 'approved_date': fields.Datetime.now()})
            
            # ======added by me to hide notification when rejected by checker=========
  
    @api.model
    def action_checker_reject(self):
        # Get the active IDs (the selected records)
        _logger.info("=====action_checker_reject called!!!!!!=====")
        active_ids = self.env.context.get('active_ids', [])
        if not active_ids:
            _logger.warning("No records selected for submission.")
        ...
        if checker_id:
            # Perform your operation on the selected records
            records.write({'status': 'checker_reject', 'type_status': 'checker_reject',
                          'user_checker': checker_id, 'checked_date': fields.Datetime.now()})  
    ...
    #     checker_id = False
    #     for record in records:
    #         if record.project_id and record.project_id.user_checker:
    #             checker_id = record.project_id.user_checker.id
    #             break
    ...
    #             'type_status': 'checker_reject',
    #             'user_checker': checker_id,
    #             'checked_date': fields.Datetime.now()
    #         })

    #         # ✅ Hide previous "Submitted" notifications for this checklist
    #         app_log_obj = self.env['app.notification.log']
    #         for rec in records:
    #             logs_to_hide = app_log_obj.search([
    #                 ('activity_type_id', '=', rec.id),
    #                 ('hide_notification', '=', False),
    #                 '|',
    #                 ('checklist_status_two', '=', 'submit'),
    #             ])
    #             if logs_to_hide:
    #                 logs_to_hide.write({'hide_notification': True})
    #                 _logger.info(
    #                     "✅ Hidden previous 'Submitted' notifications for record ID %s: %s",
    #                     rec.id, logs_to_hide.ids
    #                 )
    #             else:
    #                 _logger.info(
    #                     "ℹ️ No 'Submitted' notifications found to hide for record ID %s", rec.id
    #                 )
    ...
    def action_approver_reject(self):
        _logger.info("=====action_approver_reject called!!!!!!=====")
        ...
        if approver_id:
            ...
            records.write({'status': 'approver_reject', 'type_status': 'approver_reject',
                          'user_approver': approver_id, 'approved_date': fields.Datetime.now()})
    ...
    def _onchange_name(self):
        ...
        if activity_name:
            ...
            self.seq_no = "VB/" + str(self.project_id.name) + "/" + str(self.tower_id.name) + "/" + str(
                activity_name)+"/" + str(self.flat_id.name) + "/"+str(self.index_no)
    ...
    def _compute_progress_bar(self):
        activity_types = self.search(
            [('checklist_ids', '!=', False), ('checklist_ids.is_pass', '=', 'yes')])
        for rec in activity_types:
            checklist_ids = rec.checklist_ids
            total_lines = len(checklist_ids)
            count = len(checklist_ids.filtered(
                lambda line: line.is_pass == 'yes'))
            rec.progress_percentage = (
                count / total_lines) * 100 if total_lines > 0 and count > 0 else 0
    ...
    def button_submit(self, seq_no=None, user_id=None):
        _logger.info("----------button submit---------,%s,%s,%s",
                     self, seq_no, user_id)
        ...
        failed_log = {}
        failed_log.update(
            {'seq_no': seq_no, 'user_id': user_id, 'method': 'button_submit'})
        ...
        if user_id:
            ...
            if self.tower_id.assigned_to_ids:
                failed_log.update(
                    {'assigned_ids': self.tower_id.assigned_to_ids})
                ...
                for user in self.tower_id.assigned_to_ids:
                    ...
                    for group in groups:
                        ...
                        if str(group.name) == 'Checker':
                            ...
                            player_id, user_r = self.env['res.users'].get_player_id(
                                user.id)
                            message = "CheckList No " + \
                                str(seq_no) + " Submitted by " + \
                                str(user_record.name)
                            # _logger.info("--button submit---1----message-player_id--------,%s,%s",message,player_id)
                            e = ''
                            if player_id and message and user:
                                try:
                                    title = str(user_record.name) + \
                                        " Submitted the Checklist"
                                    log_id = notification_obj.send_push_notification(
                                        title, [player_id], message, [user.id], seq_no, 'wi', self)
                                    sent = 1
                                except Exception as e:
                                ...
                                failed_log.update(
                                    {'player_id': player_id, 'message': message, 'user': user, 'error': str(e)})
                    if group_name_not_found:
                        failed_log.update(
                            {'group_name': 'checker group name not found'})

            else:
                failed_log.update({'ids_not_found': 'Assigned Ids Not Found'})

        if not sent and not log_id:
            self.env['app.notification.log'].create(
                {'title': failed_log, 'status': 'failed'})
            
        app_log_obj = self.env['app.notification.log']
        logs_to_hide = app_log_obj.search([
            ('activity_type_id', '=', self.id),
            ('hide_notification', '=', False),
            ('res_user_id', '=', user_id),
            '|',
            ('checklist_status', 'in', ['submit', 'checker_reject']),
            ('checklist_status_two', 'in', ['submit', 'checker_reject']),
        ])
        if logs_to_hide:
            logs_to_hide.write({'hide_notification': True})
    ...
    def button_checking_done(self, seq_no=None, user_id=None):
        _logger.info(
            "----------button_checking_done-------,%s,%s", self, user_id)
        self.status = 'checked'
        self.type_status = 'checked'

        # Hide the maker-to-checker notification
        notification_logs = self.env['app.notification.log'].search([
            ('activity_type_id', '=', self.id),
            ('res_user_id', '=', user_id),  # Checker
            ('checklist_status', '=', 'submit'),
            ('hide_notification', '=', False)
        ])
        ...
        if notification_logs:
            notification_logs.write({
                ...
                'checklist_status_two': 'checked',
            })
            _logger.info("Hidden maker-to-checker notification(s) for user_id: %s", user_id)
        ...
        group_name_not_found = 1
        failed_log.update(
            {'seq_no': seq_no, 'user_id': user_id, 'method': 'button_checking_done'})
        ...
        if user_id:
            ...
            if self.tower_id.assigned_to_ids:
                ...
                for user in self.tower_id.assigned_to_ids:
                    ...
                    if group_name_not_found:
                        failed_log.update(
                            {'group_name': 'approver group name not found'})
            else:
                failed_log.update({'ids_not_found': 'Assigned Ids Not Found'})
        ...
        # Hide notifications for checkers after rejection
        app_log_obj = self.env['app.notification.log']
        logs_to_hide = app_log_obj.search([
            ('activity_type_id', '=', self.id),
            ('hide_notification', '=', False),
            ('res_user_id', '=', user_id),
            '|',
            ('checklist_status', 'in', ['submit', 'checked','approver_reject']),
            ('checklist_status_two', 'in', ['submit','checked', 'approver_reject']),
        ])
        if logs_to_hide:
            logs_to_hide.write({'hide_notification': True})
            _logger.info("Notifications hidden for logs: %s", logs_to_hide.ids)
    ...
    def button_approve(self, seq_no=None, user_id=None):
        ...
        _logger.info("----------button_approve-------,%s,%s,%s",
                     self, seq_no, user_id)
        failed_log.update(
            {'seq_no': seq_no, 'user_id': user_id, 'method': 'button_approve'})

        try:
            if self:
                rec = app_log_obj.search([('activity_type_id', '=', self.id)])
                if rec:
                    rec.write({'checklist_status': 'approve',
                              'checklist_status_two': 'approve'})


                # ✅ Hide notification after checklist is approved
                    app_log_obj.hide_checker_notifications_on_approval(self.id)
    ...
    @api.model
    def run_flag_in_activity(self, batch_size=100):
        """ Run in batches to avoid memory overflow or connection loss """
        offset = 0
        total = self.search_count([])
        _logger.info("Starting run_flag_in_activity for %s records in batches of %s", total, batch_size)

        while True:
            batch_records = self.search([], offset=offset, limit=batch_size)
            if not batch_records:
                break

            _logger.info("Processing records %s to %s", offset + 1, offset + len(batch_records))
            try:
                batch_records._compute_total_count_from_status_act_type()
                self.env.cr.commit()
            except Exception as e:
                _logger.exception("Exception during batch from %s to %s: %s", offset + 1, offset + len(batch_records), str(e))
                self.env.cr.rollback()

            offset += batch_size

        _logger.info("Finished run_flag_in_activity")
    ...
    def _compute_total_count_from_status_act_type(self):
        _logger.info(">>> _compute_total_count_from_status_act_type called <<<")
        for record in self:
            try:
                if record.checklist_ids:
                    _logger.info("Checklist count for Record ID %s: %s", record.id, len(record.checklist_ids))


                    nc = sum(record.checklist_ids.mapped('project_line_nc'))
                    orange = sum(record.checklist_ids.mapped('project_line_orange'))
                    yellow = sum(record.checklist_ids.mapped('project_line_yellow'))
                    red = sum(record.checklist_ids.mapped('project_line_red'))

                    record.act_type_nc = nc
                    record.act_type_orange = orange
                    record.act_type_yellow = yellow
                    record.act_type_red = red
            
            except Exception as e:
                _logger.exception("Error computing activity type status for record ID %s: %s", record.id, str(e))

        # commented to approve n
        # self.status='approve'

    # def check_count_for_green(self):
    #     records = self.search([('checklist_ids', '!=', False)])

    #     for record in records:
    #         total_checklist_count = len(record.checklist_ids)
    #         passed_checklist_count = len(record.checklist_ids.filtered(
    #             lambda x: x.is_pass == 'yes'  # Ensure condition matches expected values
    #         ))

    #         if total_checklist_count == passed_checklist_count:
    #             record.act_type_green = (record.act_type_green or 0) + 1
    #         else:
    #             record.act_type_green = record.act_type_green or 0

    def check_count_for_green(self):
        start = time.time()
        _logger.info(
            "=====================check_count_for_green()==============started")
        for record in self.search([('checklist_ids', '!=', False)]):
            if not record.checklist_ids:
                continue  # Skip if checklist_ids is empty or invalid
            check_list = len(record.checklist_ids)
            # _logger.info(
            #     "==================checklist=================%s", check_list)
            yes_check_list = len(record.checklist_ids.filtered(
                lambda x: x.is_pass and x.is_pass != 'yes'))
            # _logger.info(
            #     "============yes_check_list===============%s", yes_check_list)
            if check_list == yes_check_list:
                record.act_type_green += 1  # Avoid reassigning unnecessarily
        #     _logger.info("==================passed==================")
        # _logger.info("Time taken: %s seconds", time.time() - start)
        # _logger.info("=== check_count_for_green() completed ===")

    def button_set_to_draft(self, user_id=False):
        _logger.info(
            "----------button_set_to_draft-------,%s,%s", self, user_id)
        self.status = 'draft'

    def button_set_to_maker(self, seq_no=None, user_id=None):
        _logger.info(
            "---button_set_to_maker------self,seq_no,user_id---------,%s,%s,%s", self, seq_no, user_id)
        ...
        group_name_not_found = 1
        failed_log.update(
            {'seq_no': seq_no, 'user_id': user_id, 'method': 'button_set_to_maker'})
        ...
        if user_id:
            ...
            _logger.info(
                "---button_set_to_maker----self.user_maker--------,%s", self.user_maker)
            ...
            if not self.user_maker:
                ...
                if self.tower_id.assigned_to_ids:
                    ...
                    for user in self.tower_id.assigned_to_ids:
                        ...
                        for group in groups:
                            if str(group.name) == 'Maker':
                                ...
                                player_id, user_r = self.env['res.users'].get_player_id(
                                    user.id)
                                message = "CheckList No " + \
                                    str(seq_no) + " Rejected by " + \
                                    str(user_record.name)
                                e = ''
                                if player_id and message and user:
                                    try:
                                        title = str(user_record.name) + \
                                            " Rejected the Checklist"
                                        log_id = notification_obj.send_push_notification(
                                            title, [player_id], message, [user.id], seq_no, 'wi', self)
                                        sent = 1
                                    except Exception as e:
                                        _logger.info(
                                            "--wi1--button_set_to_maker---1---exception---------,%s", str(e))
                                        pass
                                else:
                                    failed_log.update(
                                        {'player_id': player_id, 'message': message, 'user': user, 'error': str(e)})
                        if group_name_not_found:
                            failed_log.update(
                                {'group_name': 'maker group name not found'})
                else:
                    failed_log.update(
                        {'ids_not_found': 'Assigned Ids Not Found'})
            else:
                player_id, user_r = self.env['res.users'].get_player_id(
                    self.user_maker.id)
                message = "CheckList No " + \
                    str(seq_no) + " Rejected by " + str(user_record.name)
                if player_id and message and user_record:
                    try:
                        title = str(user_record.name) + \
                            " Rejected the Checklist"
                        log_id = notification_obj.send_push_notification(
                            title, [player_id], message, [self.user_maker.id], seq_no, 'wi', self)
                        sent = 1
                    except Exception as e:
                        _logger.info(
                            "--wi2--button_set_to_maker---1---exception---------,%s", str(e))
                        pass
                else:
                    failed_log.update(
                        {'player_id': player_id, 'message': message, 'user': user})
        ...
        logs_to_hide = app_log_obj.search([
            ...
            ('res_user_id', '=', user_id),
             '|',
            ('checklist_status', 'in', ['submit', 'checker_reject','approver_reject']),
            ('checklist_status_two', 'in', ['submit', 'checker_reject','approver_reject']),
        ...
        self.action_checker_reject()

        return

    def button_set_to_checker(self, seq_no=None, user_id=None):
        _logger.info(
            "---button_set_to_checker------self,seq_no,user_id---------,%s,%s,%s", self, seq_no, user_id)
    ...
    # 👇 Only track rejection reason in type_status
        self.status = 'approver_reject'
        ...
        if user_id:
            ...
            if not self.user_checker:
                if self.project_id.assigned_to_ids:
                    failed_log.update(
                        {'assigned_ids': self.project_id.assigned_to_ids})
                ...
                if player_id and message and user_record:
                    try:
                        title = str(user_record.name) + \
                            " Rejected the Checklist"
                        log_id = notification_obj.send_push_notification(
                            title, [player_id], message, [self.user_checker.id], seq_no, 'wi', self)
                        sent = 1
                    except Exception as e:
                        _logger.info(
                            "--wi3--button_set_to_maker---1---exception---------,%s", str(e))
                        pass
                else:
                    failed_log.update(
                        {'player_id': player_id, 'message': message, 'user': user})
        ...
        # Hide notifications for approvers after rejection
        app_log_obj = self.env['app.notification.log']
        logs_to_hide = app_log_obj.search([
            ('activity_type_id', '=', self.id),
            ('hide_notification', '=', False),
        ...
        self.action_approver_reject()
        
        return

    def get_project_activity_details(self, activity_type_id):
        _logger.info(
            "----------get_project_activity_details-------,%s", activity_type_id)
        ...
        try:
            ...
            _logger.info(
                "------get_project_activity_details--reject-----,%s", str(e))
            pass
        ...
        ###
            ...
            _logger.info(
                "-get_project_activity_details--history-----,%s", str(e))
            pass
        ...
        try:
            if activity.activity_type_img_ids:
                for img in activity.activity_type_img_ids:
                    if img.img_type == 'pat':
                        checklist_image_url = base_url + \
                            "/web/image?model=project.activity.type.image&field=overall_img&id=" + \
                            str(img.id)
...
class ProjectChecklistLine(models.Model):
    ...
    image_ids = fields.One2many(
        "project.checklist.line.images", 'checklist_line_id', string="Images")
    ...
    # project_line_cre_date = fields.Datetime(
    # 'Checklist Create Date', default=lambda self: fields.Datetime.now() + timedelta(hours=0, minutes=0))
    project_line_cre_date = fields.Datetime(
        'Checklist Create Date', default=lambda self: fields.Datetime.now() + timedelta(hours=5, minutes=30))
    image = fields.Binary()
    img_desc = fields.Char(string='Image Description') 
    ...
    def is_days_difference_for_status_count(self):
        """Update project line status counters based on the days difference (only if is_pass = no)."""
        today_date = datetime.now()
        _logger.info("⏱ Starting NC status update job at: %s", today_date)

        limit = 100
        records = self.search([('project_line_cre_date', '!=', False)], limit=limit)

        for rec in records:
            # ✅ Only process if is_pass == 'no'
            if rec.is_pass != 'no':
                continue

            if rec.activity_type_id and rec.activity_type_id.status == 'rejected':
                target_date = rec.project_line_cre_date
                calculated_days_difference = (today_date - target_date).days

                # Reset flags
                rec.project_line_nc = 0
                rec.project_line_yellow = 0
                rec.project_line_orange = 0
                rec.project_line_red = 0

                is_changed = False

                if calculated_days_difference in [2, 4, 7, 14]:
                    if calculated_days_difference == 2:
                        rec.project_line_nc = 1
                        _logger.info("✔ Record ID %s | NC set (Day 2)", rec.id)
                    elif calculated_days_difference == 4:
                        rec.project_line_yellow = 1
                        _logger.info("✔ Record ID %s | Yellow flag set (Day 4)", rec.id)
                    elif calculated_days_difference == 7:
                        rec.project_line_orange = 1
                        _logger.info("✔ Record ID %s | Orange flag set (Day 7)", rec.id)
                    elif calculated_days_difference == 14:
                        rec.project_line_red = 1
                        _logger.info("✔ Record ID %s | Red flag set (Day 14)", rec.id)

                    is_changed = True

                if is_changed:
                    activity_type_ids = self.env['project.activity.name.line'].search([
                        ('checklist_ids', 'in', rec.id),
                        ('status', '=', 'rejected')
                    ])
                    activity_type_ids._compute_total_count_from_status_act_type()

                    activity_ids = self.env['project.activity.name'].search([
                        ('activity_type_ids', 'in', activity_type_ids.ids)
                    ])
                    rec.activity_type_id.activity_id._compute_total_count_from_status_act()

                    floor_ids = self.env['project.floors'].search([
                        ('activity_ids', 'in', activity_ids.ids)
                    ])
                    floor_ids._compute_total_count_from_status_floors()

                    flat_ids = self.env['project.flats'].search([
                        ('activity_ids', 'in', activity_ids.ids)
                    ])
                    flat_ids._compute_total_count_from_status_flats()

                    tower_ids = self.env['project.tower'].search([
                        '|',
                        ('tower_floor_line_id', 'in', floor_ids.ids),
                        ('tower_flat_line_id', 'in', flat_ids.ids)
                    ])
                    tower_ids._compute_total_count_from_status_tower()

                    activity_id = rec.activity_type_id.activity_id

                    self.env['manually.set.flag'].create({
                        'project_info_id': activity_id.project_id.id,
                    ...
                    _logger.info("🟢 Manually flag created for Record ID %s (Activity ID: %s)", rec.id, activity_id.id)

        _logger.info("✅ NC status update job completed.")
    ...
    #     today_date = datetime.now()
    #     records = self.search(
    #         [('is_pass', '=', 'no'), ('project_line_cre_date', '!=', False)])
    #     _logger.info("====records found====")
    #     for rec in records:
    #         is_changed = False
    #         if rec.activity_type_id and rec.activity_type_id.status in ['rejected']:
    #             target_date = rec.project_line_cre_date
    ...
    #             if is_changed:
    #                 activity_type_ids = self.env['project.activity.name.line'].search(
    #                     [('checklist_ids', 'in', rec.id), ('status', 'in', ['rejected'])])
    #                 activity_type_ids._compute_total_count_from_status_act_type()
    #                 activity_ids = self.env['project.activity.name'].search(
    #                     [('activity_type_ids', 'in', activity_type_ids.ids)])
    #                 rec.activity_type_id.activity_id._compute_total_count_from_status_act()
    ...
    #                 flat_ids._compute_total_count_from_status_flats()
    #                 tower_ids = self.env['project.tower'].search(
    #                     ['|', ('tower_floor_line_id', 'in', floor_ids.ids), ('tower_flat_line_id', 'in', flat_ids.ids)])
    #                 tower_ids._compute_total_count_from_status_tower()
    ...
    def onchange_status_for_mail(self):
        if self.is_pass and self.is_pass == 'yes':
            mail_template = self.env.ref(
                'custom_project_management.email_status_set_to_yes_mail')
            ...
            mail_mail.send()
        elif self.is_pass and self.is_pass == 'no':
            mail_template = self.env.ref(
                'custom_project_management.email_status_set_to_no_mail')
            mail_mail = self.env['mail.mail'].create({
...
class ProjectChecklistLineImages(models.Model):
    ...
    project_checklist_line_log_id = fields.Many2one(
        'project.checklist.line.log')
...
class ProjectChecklistTemplate(models.Model):
    ...
    name = fields.Char('Name')
    # log_ids = fields.One2many('project.checklist.template.log', 'checklist_template_id', string='Logs')
    # is_pass = fields.Selection([('yes', 'Yes'), ('no', 'No'), ('nop', 'Not Applicable')], 
    #                            string="Latest Status", compute="_compute_is_pass", store=True)

    # @api.depends('log_ids.is_pass')
    # def _compute_is_pass(self):
    #     for rec in self:
    #         if rec.log_ids:
    #             # You can customize this logic as needed
    #             latest_log = rec.log_ids.sorted(key=lambda r: r.id, reverse=True)[0]
    #             rec.is_pass = latest_log.is_pass
    #         else:
    #             rec.is_pass = False
